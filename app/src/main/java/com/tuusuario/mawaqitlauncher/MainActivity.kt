...
class MainActivity : Activity() {
    ...
    private fun launchMawaqit() {
        try {
            val viewIntent = Intent(Intent.ACTION_VIEW, Uri.parse(MAWAQIT_URL))
            viewIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            // Si la app Mawaqit tiene registrado este enlace, Android la abre directo
            // en esa mezquita. Si no, se abre en el navegador con la misma página.
            viewIntent.setPackage(MAWAQIT_PACKAGE)
            val resolved = packageManager.resolveActivity(viewIntent, PackageManager.MATCH_DEFAULT_ONLY)
            if (resolved != null) {
                startActivity(viewIntent)
            } else {
                // La app no maneja este enlace: abrirlo sin restringir el paquete (navegador)
                val openIntent = Intent(Intent.ACTION_VIEW, Uri.parse(MAWAQIT_URL))
                openIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(openIntent)
            }
            LaunchLog.record(this, "app")
        } catch (e: Exception) {
            // Último recurso: abrir la app de Mawaqit sin especificar mezquita
