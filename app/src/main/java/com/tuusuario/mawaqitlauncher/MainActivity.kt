...
class MainActivity : Activity() {
    ...
    private val MAWAQIT_PACKAGE = "com.mawaqit.androidtv"
    ...
    /**
     * En Android TV, el mando mueve el "foco" entre botones, pero sin un
     * indicador visual no se nota nada al pulsar las flechas. Esto hace
     * que el botón se agrande y resalte cuando el mando lo selecciona.
     */
    private fun makeTvFocusable(view: View) {
        ...
        when (step) {
            ...
            Step.BATTERY -> {
                text.text = "Paso 2 de 3\n\nDesactiva la optimización de batería para\n" +
                        ...
                        "(Como la tele siempre está enchufada, este paso\n" +
                        "es solo un extra — si no funciona, puedes saltarlo\n" +
                        "sin problema.)"
                ...
                button.setOnClickListener {
                    ...
                    if (!opened) {
                        text.text = "Este dispositivo no tiene esa pantalla disponible.\n\n" +
                                "No pasa nada: como la tele siempre está enchufada,\n" +
                                "puedes saltar este paso sin problema."
                    }
                }
            }
            Step.SCREEN -> {
                ...
                button.setOnClickListener {
                    ...
                    if (!opened) {
                        text.text = "Este dispositivo no tiene esa pantalla disponible.\n\n" +
                                "Puedes saltar este paso; revisa en Ajustes de la\n" +
                                "propia tele que el protector de pantalla esté\n" +
                                "desactivado, para que no tape a Mawaqit."
                    }
                }
            }
        }
    ...
    private fun launchMawaqit() {
        val launchIntent = packageManager.getLeanbackLaunchIntentForPackage(MAWAQIT_PACKAGE)
            ?: packageManager.getLaunchIntentForPackage(MAWAQIT_PACKAGE)
