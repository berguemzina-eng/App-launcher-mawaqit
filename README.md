...
4. Un asistente de configuración de **3 pasos** (solo la primera vez) deja todo listo para uso 24/7:
   - Paso 1: activar Accesibilidad (TV) o Inicio predeterminado (móvil).
   - Paso 2: excluir la app de la optimización de batería.
   - Paso 3: permitir que la app fije la pantalla para que nunca se apague.
5. **Al abrir la app manualmente** (no durante el arranque automático), se muestra una pantalla de bienvenida con el logotipo de mezquita animado (cúpula, minaretes, luna y estrella) durante ~1.8 segundos, con un botón "Ajustes" para acceder a más opciones sin esperar.
6. **Pantalla de Ajustes completa**: estado de cada permiso (✓/✗), botones para reconfigurar cada paso individualmente, activar/desactivar "pantalla siempre encendida", y acceso directo para abrir Mawaqit al instante.
7. **Reintentos automáticos al arrancar**: si Mawaqit aún no está listo justo tras el arranque, el receptor reintenta varias veces antes de rendirse.
8. **Vigilante permanente**: mientras el servicio de accesibilidad esté activo, cualquier pantalla que no sea Mawaqit (el launcher de Xiaomi, un diálogo de error, un cierre inesperado) dispara el relanzamiento automático de Mawaqit.
...
10. **Accesos directos** en Ajustes a Wi-Fi, fecha/hora y protector de pantalla del sistema.

## Requisitos

- Una cuenta de GitHub (gratis).
- Tener instalada la app oficial de **Mawaqit** en el dispositivo (paquete `com.mawaqit.androidtv`).
